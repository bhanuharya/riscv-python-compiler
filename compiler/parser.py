...
class ASTVisitor(ast.NodeVisitor):
    ...
    def visit_For(self, node):
        ...
        # Ensure the loop is over a range or xrange
        if not (hasattr(node.iter, 'func') and node.iter.func.id in ['xrange', 'range']):
            raise RuntimeError('Loop must be over range or xrange')
        ...
        # Determine the start, stop, and step based on the number of arguments
        if len(args) == 1:  # range(stop)
            ...
            raise RuntimeError('range() expects 1-3 arguments, got {}'.format(len(args)))
    ...
    def visit_Compare(self, node):
        # Function to get the operation name from the node class
        def visit_op(sub_node):
            op_type = sub_node.__class__
            if op_type not in PRIM_OPS:
                raise RuntimeError(f"Unsupported operation: {op_type}")
            return PRIM_OPS[op_type]

        # Visit the left side of the comparison
        left = self.visit(node.left)

        # Visit each comparison operator and comparator
        ops = [visit_op(op) for op in node.ops]
